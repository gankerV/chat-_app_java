...
public class MessageDAO {
    ...
    public List<String> getGroupChatHistory(int groupId) throws SQLException {
        String sql = "SELECT mg.TIME_SEND, ua.USERNAME, mg.CONTENT " +
                     "FROM MESSAGE_GROUP mg " +
                     "JOIN USER_ACCOUNT ua ON mg.FROM_USER = ua.ID " +
                     "WHERE mg.TO_GROUP = ? " +
                     "ORDER BY mg.TIME_SEND ASC";
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            ResultSet rs = stmt.executeQuery();
            while (rs.next()) {
                String message = "[" + rs.getTimestamp("TIME_SEND") + "] " +
                                 rs.getString("USERNAME") + ": " +
                                 rs.getString("CONTENT");
    ...
    public void addMessageToGroup(int userId, int groupId, String message) throws SQLException {
        String sql = "INSERT INTO MESSAGE_GROUP (FROM_USER, TO_GROUP, TIME_SEND, CONTENT) " +
    ...
    //Xóa toàn bộ tin nhắn trong nhóm
    public void deleteAllMessagesInGroup(int groupId) throws SQLException {
        String sql = "DELETE FROM MESSAGE_GROUP WHERE TO_GROUP = ?";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, groupId);
            stmt.executeUpdate();
        }
    ...
    //Xóa tin nhắn mới nhất trong nhóm
    public void deleteLatestMessageInGroup(int groupId) throws SQLException {
        String sql = """
